...
void print_network_information(void)
{
    ...
    wizchip_getnetinfo(0, &net_info); // Get chip configuration information
    ...
    while (1)
    {
        ...
        if (dhcp_run_flag == 0)
        {
            ...
            if (dhcp_ok_flag)
            {
                ...
                getSHAR(0, conf_info.mac);
                wizchip_setnetinfo(0, &conf_info); // Update network information to network information obtained by DHCP
                return 1;
            }
            return 0;
        }
 ...
 * @return  none
 */
void network_init(uint8_t chipNum, uint8_t *ethernet_buff, wiz_NetInfo *conf_info)
{
    int ret;
    wizchip_setnetinfo(chipNum, conf_info); // Configuring Network Information
    if (conf_info->dhcp == NETINFO_DHCP)
    {
        ...
        if (ret == 0)
        {
            ...
            conf_info->ip[2] = 1;
            conf_info->ip[3] = 100;
            wizchip_setnetinfo(chipNum, conf_info);
        }
    }
